...
@register("astrbot_plugin_6657langen", "6657bot", "从 sb6657.cn 获取随机烂梗或按关键词搜索烂梗", "1.0.1")
class LangenPlugin(Star):
    ...
    async def _get_session(self) -> aiohttp.ClientSession:
        """获取共享的 HTTP 会话，支持连接复用"""
        if self._session is None or self._session.closed:
            self._session = aiohttp.ClientSession(
                timeout=REQUEST_TIMEOUT,
                headers={"User-Agent": "AstrBot-6657LangenPlugin/1.0"},
            )
    ...
    async def random_langen(self, event: AstrMessageEvent):
        """随机获取一条烂梗"""
        try:
            data = await self._fetch_json(f"{BASE_URL}/machine/getRandOne")
    ...
    async def search_langen(self, event: AstrMessageEvent, keyword: str = ""):
        ...
        if len(keyword) > MAX_KEYWORD_LENGTH:
            yield event.plain_result(f"关键词过长，请控制在 {MAX_KEYWORD_LENGTH} 字以内")
            return

        kw = keyword.lower()

        # 读取配置
        search_mode = self._conf("search_mode", "full")
        page_size = self._conf("page_size", DEFAULT_PAGE_SIZE)
        concurrent_limit = self._conf("concurrent_limit", DEFAULT_CONCURRENT_LIMIT)
        search_pages = self._conf("search_pages", DEFAULT_SEARCH_PAGES)
