...
/搜梗 donk
🔍 搜索「donk」在全站 17000 条烂梗中找到 5 条相关结果，随机一条：
#8075 donk：载大哥我又坐牢了...
```

## 配置

在 AstrBot 管理面板 → 插件管理 → 6657烂梗库 中可调整以下选项：

| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| 搜索模式 | 全站搜索（完整但较慢） / 随机抽样搜索（快速） | 全站搜索 |
| 随机搜索页数 | 抽样模式下扫描的页数（仅抽样模式时显示） | 10 |
| 每页请求数量 | 单次 API 请求获取的条目数，建议 50-200 | 200 |
| 并发请求上限 | 同时发起的最大请求数，建议 3-10 | 8 |
